...
export const saveSelectedCourses = async (
  googleId: string,
...
export const getUserCourses = async (googleId: string): Promise<ICourse[]> => {
  const user = await User.findOne({ googleId });
  if (!user) throw new Error("User not found");
...
export const saveCourseMaterials = async (
  googleId: string,
  courseId: string,
  materials: { title: string; mimeType: string; contentBuffer?: Buffer }[],
  authClient: any
): Promise<IMaterial[]> => {
